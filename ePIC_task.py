...
electron_count, electron_count_pos, electron_count_neg, electron_count_f = 0, 0, 0, 0
pion_count, pion_count_pos, pion_count_neg, pion_count_f = 0, 0, 0, 0
kaon_count, kaon_count_pos, kaon_count_neg, kaon_count_f = 0, 0, 0, 0
proton_count, proton_count_pos, proton_count_neg, proton_count_f = 0, 0, 0, 0
neutron_count, neutron_count_pos, neutron_count_neg, neutron_count_f = 0, 0, 0, 0
rho_count, rho_count_pos, rho_count_neg, rho_count_f = 0, 0, 0, 0
gluon_count, gluon_count_pos, gluon_count_neg, gluon_count_f = 0, 0, 0, 0
photon_count, photon_count_pos, photon_count_neg, photon_count_f = 0, 0, 0, 0
other_count, other_count_pos, other_count_neg, other_count_f = 0, 0, 0, 0
up_count, up_count_pos, up_count_neg, up_count_f = 0, 0, 0, 0
down_count, down_count_pos, down_count_neg, down_count_f = 0, 0, 0, 0
charm_count, charm_count_pos, charm_count_neg, charm_count_f = 0, 0, 0, 0
strange_count, strange_count_pos, strange_count_neg, strange_count_f = 0, 0, 0, 0
diq_count, diq_count_pos, diq_count_neg, diq_count_f = 0, 0, 0, 0
diq2_count, diq2_count_pos, diq2_count_neg, diq2_count_f = 0, 0, 0, 0
delta_count, delta_count_pos, delta_count_neg, delta_count_f = 0, 0, 0, 0
lambda_count, lambda_count_pos, lambda_count_neg, lambda_count_f = 0, 0, 0, 0
epsilon_count, epsilon_count_pos, epsilon_count_neg, epsilon_count_f = 0, 0, 0, 0
eta_count, eta_count_pos, eta_count_neg, eta_count_f = 0, 0, 0, 0
omegaphi_count, omegaphi_count_pos, omegaphi_count_neg, omegaphi_count_f = 0, 0, 0, 0
frag_count, frag_count_pos, frag_count_neg, frag_count_f = 0, 0, 0, 0
difractive_count, difractive_count_pos, difractive_count_neg, difractive_count_f = 0, 0, 0, 0
strangeK_count, strangeK_count_pos, strangeK_count_neg, strangeK_count_f = 0, 0, 0, 0
xi_count, xi_count_pos, xi_count_neg, xi_count_f = 0, 0, 0, 0
...
# PION
y_DA_pi = [np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) for p, t in zip(pion_angles, lepton_angles) if 0.001 <= np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) <= 0.95]
Q2_DA_pi = [np.abs(4*18*18*(1-abs(y))/(np.tan(t/2)**2)) for y, t in zip(y_DA_pi, lepton_angles)]
x_DA_pi = [Q/(4*18*275*y*275) for Q, y in zip(Q2_DA_pi, y_DA_pi) if Q/(4*18*275*y*275)]
# PROTON
y_DA_proton = [np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) for p, t in zip(proton_angles, lepton_angles) if 0.001 <= np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) <= 0.95]
Q2_DA_proton = [np.abs(4*18*18*(1-abs(y))/(np.tan(t/2)**2)) for y, t in zip(y_DA_proton, lepton_angles)]
x_DA_proton = [Q/(4*18*275*y*275) for Q, y in zip(Q2_DA_proton, y_DA_proton)]
# KAON
y_DA_kaon = [np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) for p, t in zip(kaon_angles, lepton_angles) if 0.001 <= np.abs(np.tan(p/2)/(np.tan(p/2) + np.tan(t/2))) <= 0.95]
Q2_DA_kaon = [np.abs(4*18*18*(1-abs(y))/(np.tan(t/2)**2)) for y, t in zip(y_DA_kaon, lepton_angles)]
x_DA_kaon = [Q/(4*18*275*y*275) for Q, y in zip(Q2_DA_kaon, y_DA_kaon)]
'''
